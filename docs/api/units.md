...
| `Time.hpp` | `Time`, `Frequency` (= `AngularVelocity`) | `Time`, `Frequency`, `AngularVelocity` | `second`, `millisecond`, `microsecond`, `nanosecond`, `minute`, `hour`, `day`, `year` (Julian), `megayear`, `gigayear`, `hertz` | `_s _ms _min _h _day _yr _Myr _Gyr _Hz` (collide with `std::chrono_literals`, deliberately; see below) |
