...
Derivations and the analytic results the test suite validates against.
Interface documentation is in [src/Physics/README.md](../src/Physics/README.md);
this is the material too long for a header comment.

Built in stages; this document covers what exists so far.
...
- [The 1PN correction](#the-1pn-correction)
- [Spacetime conventions](#spacetime-conventions)
...
is written in. See [docs/units.md](units.md).
...
Per docs/architecture.md, light is a null geodesic; `Optics/Propagation.hpp`
adds nothing to the geodesic solver beyond building the right initial
tangent and running it. `nullTangent(metric, at, direction)` fixes the
tangent's magnitude by solving `g_mu_nu u^mu u^nu = 0`, quadratic in `u^0`
given the spatial components `direction`:
